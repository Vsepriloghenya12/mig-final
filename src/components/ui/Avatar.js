...
export function Avatar({ user, size = 42, ringColor }) {
  ...
  return (
    <View style={[{ width: size, height: size, borderRadius: size / 2, overflow: 'hidden', backgroundColor: colors.faint }, border]}>
      {user?.avatarUrl || user?.avatarKey ? <Image source={mediaSource(user, 'avatar')} style={{ width: '100%', height: '100%' }} /> : (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: colors.ink, fontWeight: '900' }}>{initials(user?.name)}</Text></View>
      )}
    </View>
  );
}
