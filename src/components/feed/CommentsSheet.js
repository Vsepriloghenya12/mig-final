...
export function CommentsSheet({ post, visible, onClose, onSend }) {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} accessibilityViewIsModal>
      <Pressable accessibilityRole="button" accessibilityLabel="Закрыть комментарии" style={styles.backdrop} onPress={onClose} />
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.keyboardWrap}>
        <View style={styles.sheet}>
          <View style={styles.header}><Text style={styles.title}>Комментарии</Text><Pressable accessibilityRole="button" accessibilityLabel="Закрыть" hitSlop={10} onPress={onClose}><Icon name="close" size={25} /></Pressable></View>
          <ScrollView style={{ maxHeight: 320 }} contentContainerStyle={styles.list} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.inputRow}><TextInput accessibilityLabel="Текст комментария" value={text} onChangeText={setText} placeholder="Написать комментарий" placeholderTextColor={colors.muted} style={styles.input} /><Pressable accessibilityRole="button" accessibilityLabel="Отправить комментарий" onPress={send} style={[styles.send, !text.trim() && styles.sendDisabled]}><Icon name="send" color={colors.white} size={18} /></Pressable></View>
        </View>
...
const styles = StyleSheet.create({
  ...
  sheet: { backgroundColor: colors.white, borderTopLeftRadius: 30, borderTopRightRadius: 30, paddingBottom: 22, overflow: 'hidden' },
