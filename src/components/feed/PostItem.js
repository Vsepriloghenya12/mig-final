...
export function PostItem({ post, onLike, onSave, onComment, onOpenProfile, onReport, onBlock }) {
  ...
  return <View style={styles.post}>
    <View style={styles.authorRow}>
      <Pressable onPress={() => onOpenProfile?.(post.author)} style={styles.authorTap}><Avatar user={post.author} size={38} /><View style={{ flex: 1 }}><Text style={styles.handle}>{post.author?.handle || post.author?.name}</Text><Text style={styles.meta}>{post.location || 'Миг'} · {post.timeLabel || ''}</Text></View></Pressable>
      <Pressable onPress={onReport} hitSlop={10}><Icon name="more" size={18} color={colors.muted} /></Pressable>
    </View>
    ...
    <View style={styles.actions}>
      ...
      <View style={{ flex: 1 }} />
      <Pressable onPress={onBlock} style={styles.block}><Text style={styles.blockText}>Блок</Text></Pressable>
...
const styles = StyleSheet.create({
  ...
  block: { paddingHorizontal: 8, height: 28, borderRadius: 14, backgroundColor: colors.faint, alignItems: 'center', justifyContent: 'center' },
  blockText: { color: colors.muted, fontSize: 11, fontWeight: '900' },
