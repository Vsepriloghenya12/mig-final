...
app.listen(PORT, () => console.log(`Blizz backend on :${PORT}; data ${DATA_DIR}`));

function homeHtml() {
  return `<!doctype html><html lang="ru"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Близз backend</title><style>body{margin:0;font-family:system-ui;background:#fbfaff;color:#15142d}.wrap{min-height:100vh;display:grid;place-items:center;padding:28px}.card{max-width:720px;background:white;border:1px solid #eee8fa;border-radius:30px;padding:32px;box-shadow:0 24px 70px #3120701f}.logo{width:72px;height:72px;border-radius:24px;background:linear-gradient(135deg,#ff6b6b,#f22d8f,#7b5cff,#2f7bff)}a{display:inline-flex;margin-top:20px;background:#f22d8f;color:white;text-decoration:none;padding:13px 18px;border-radius:18px;font-weight:800}</style><div class="wrap"><div class="card"><div class="logo"></div><h1>Близз backend</h1><p>Сервер работает. Health: <code>/api/health</code></p><a href="/owner">Страница владельца</a><a style="margin-left:10px" href="/privacy">Privacy</a><a style="margin-left:10px" href="/terms">Terms</a><a style="margin-left:10px" href="/support">Support</a><a style="margin-left:10px" href="/delete-account">Удаление аккаунта</a></div></div>`;
}
