...
function passwordHash(value = '') { return crypto.createHash('sha256').update(String(value || '')).digest('hex'); }
...
router.post('/users', (req, res) => {
  ...
  if (mode === 'login') {
    if (password.length < 6) return res.status(400).json({ error: 'Введите пароль.' });
  ...
  user.firstName = clean(req.body.firstName, user.firstName || '');
  user.lastName = clean(req.body.lastName, user.lastName || '');
  user.phone = phone || user.phone || '';
  if (handle) user.handle = handle;
  if (mode === 'register') user.passwordHash = passwordHash(password);
  user.status ||= 'active';
